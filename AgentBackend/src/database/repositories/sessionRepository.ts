...
export class SessionRepository {
  ...
  /**
   * Get a session with its hook code
   * @param id Session ID
   * @returns The session with hook code or null if not found
   */
  async getSessionWithHookCode(id: string): Promise<Prisma.SessionGetPayload<{
    include: { hookCode: true }
  }> | null> {
    return prisma.session.findUnique({
      where: { id },
      include: {
  ...
  async getCompleteSession(id: string): Promise<Prisma.SessionGetPayload<{
    include: { messages: true; hookCode: true }
  }> | null> {
    return prisma.session.findUnique({
      where: { id },
      include: {
